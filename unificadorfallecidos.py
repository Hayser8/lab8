...
# ============================
INPUT_DIR = Path("datafallecidos")
OUT_DIR = Path("out")
UNIFY_FILENAME = "fallecidos_unificado.csv"
...
def extract_year_from_name(path: Path) -> Optional[int]:
    ...
    Ejemplos válidos: 2019, 2020, 1998, 'defunciones_2021.sav', 'Fallecidos-2022.xlsx'
    """
    ...
    if candidates:
        try:
            ...
            return None
    # como fallback: busca en carpeta padre
...
def normalize_col(col: str) -> str:
    ...
    """
    col = col.strip()
    col = unidecode(col)  # remueve acentos
    col = col.lower()
    col = re.sub(r'[^\w\s]+', '_', col)  # símbolos raros -> _
    col = re.sub(r'\s+', '_', col)       # espacios -> _
    col = re.sub(r'_+', '_', col)        # colapsa múltiples _
    col = col.strip('_')
    return col
...
def read_file_any(path: Path) -> Tuple[pd.DataFrame, Optional[object]]:
    ...
    if suffix == '.sav':
        try:
            import pyreadstat  # import diferido para evitar fallo si no está instalado
        except ImportError:
            raise RuntimeError(
                f"Se encontró un .sav ({path.name}) pero falta pyreadstat.\n"
                f"Instala con: pip install pyreadstat==1.2.7"
            )
        df, meta = pyreadstat.read_sav(str(path), apply_value_formats=False)  # mantenemos códigos numéricos
        return df, meta
    elif suffix in ('.xlsx', '.xls'):
        df = pd.read_excel(path, dtype=object)  # dtype=object para no forzar tipos
        return df, None
    else:
...
def process_folder(input_dir: Path, out_dir: Path, unify_filename: str) -> None:
    ...
    for f in files:
        ...
        if 'anio' not in df.columns:
            ...
            # si existe, intenta completar nulos con year
        ...
        # versión Markdown breve (útil para revisión rápida)
